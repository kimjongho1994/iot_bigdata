...
//const 의 이름은 모두 대문자로 쓰는 것이 관례(Naming Convention)이다.
	const unsigned long long MY_PASSWORD=23162456454646467;
	unsigned long long input_password=0;
	const int MY_MAX;
	// MY_MAX의 값을 최초로 정의(define)하지만 그 이전의 MY_MAX가 선언되는
	// 과정에서 초기화되었으므로 아래 라인은 const 값의 변경을 시도하는
	// 것이기 대문에 컴파일 에러를 발생한다.
