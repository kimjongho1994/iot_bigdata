...
// 전체 기능: 두 수를 입력받아서 절대 값이 큰 수를 판단해주는 프로그램
...
//수행순서 : 8,9,10,11,15,26,31,38,39,40,31,38,39,41,42,31,32,15,24
void main(){
	int num1, num2;
	printf("두 개의 정수 입력: ");
	scanf("%d %d", &num1, &num2);
	...
	// 다른 언어에서는 문제가 될 수 있다. (ex, Python)
	printf("%d와 %d중 절댓값이 큰 정수: %d \n",
		       	num1, num2, AbsoCompare(num1, num2));
	
	// printf함수 첫번째 인자인 문자열에서 ""\n을 사용하지않고
	// 줄바꿈을 할 경우에는 컴파일에러를 발생한다.
	/*
	printf("%d와 %d중 
	   	절댓값이 큰 정수: %d \n",
		num1, num2, AbsoCompare(num1, num2));
...
int AbsoCompare(int num1, int num2){
	// 함수는 main 함수에서만 호출할 수 있는 것이 아니다.
	// 함수에서 함수를 호출할 수 있다.
	// 관계연산을 하기 위해서 좌항, 우항의 함수를 각각 호출하여 
	// 평가한 결과를 기반으로 비교연산을 한다.
